...
foreach ($_SERVER as $key => $value) {
    if (strpos($key, 'HTTP_') === 0  ||  strpos($key, 'CONTENT_') === 0) {
        ...
        if ($headername == 'Sec-Fetch-Mode' && $value == 'cors') {
            $is_cors = True;
        }
    }        
}
...
if (isset($_REQUEST['y'])) {
    $request_url = urldecode($_REQUEST['y']);
...
if (is_array($request_params) && array_key_exists('y', $request_params)) {
    unset($request_params['y']);
}
...
if (count($request_params) > 0 && (!array_key_exists('query', $p_request_url) || empty($p_request_url['query']))) {
...
foreach ($response_headers as $key => $response_header) {
    //print($response_header . "<br>");
    
    // Rewrite the `Location` header, so clients will also use the proxy for redirects.
    if (preg_match('/(?i)^Location:/', $response_header)) {
        list($header, $value) = preg_split('/: /', $response_header, 2);
        $response_header = 'Location: ' . preg_replace('/\?y=.*/', '?y='.urlencode($value), $_SERVER['REQUEST_URI']);
    }
